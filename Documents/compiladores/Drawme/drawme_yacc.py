...
def p_functions(p): 
	...
	proDict[avail.getScope()][7] = temps[3]
	ht.clear()
	avail.function_end()
...
def dict_to_string(convDict):
	...
	for key in convDict:
		...
		if(key == 'main' or key == 'globals'):
			s += str(key) + ' ' + str(info[0]) + ' ' + str(info[1]) + ' ' + str(info[2]) + ' ' + str(info[3]) + ' ' + str(info[4]) + ' ' + str(info[5]) + '\n'
		else:
			s += str(key) + ' ' + str(info[2]) + ' ' + str(info[3]) + ' ' + str(info[4]) + ' ' + str(info[5]) + ' ' + str(info[6]) + ' ' + str(info[7]) + '\n'
	s += '%%' + '\n'
	return s
